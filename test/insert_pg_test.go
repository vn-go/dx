...
func TestInsertUserPG(t *testing.T) {
...
func BenchmarkInsertUserBG(t *testing.B) {
	db, err := dx.Open("postgres", pgDsn)
	assert.NoError(t, err)
	defer db.Close()
	for i := 0; i < t.N; i++ {
		...
		for j := 0; j < 10000; j++ {
			...
			user.Username = fmt.Sprintf("user-e-%d", j+i*10)
			err := db.Insert(user)
			assert.NoError(t, err)
		}
...
func TestInsertUserWithContextPG(t *testing.T) {
	user, err := dx.NewDTO[models.User]()
	user.Username = "user12345"
	assert.NoError(t, err)
	db, err := dx.Open("postgres", pgDsn)
