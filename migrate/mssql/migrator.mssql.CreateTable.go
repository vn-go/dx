...
func (m *MigratorMssql) GetSqlCreateTable(typ reflect.Type) (string, error) {
	...
	schema, err := schemaLoader.LoadFullSchema(m.Db) //<-- Load schema from the database. LoadFullSchema is called only once per database
	if err != nil {
		return "", err
	}
	...
	for _, col := range entityItem.Entity.Cols {
		...
		if col.Default != "" {
			defaultVal, err := common.TypeUtils.GetDefaultValue(col.Default, defaultValueByFromDbTag)
			if err != nil {
				err = fmt.Errorf("not support default value from %s, review %s or GetGetDefaultValueByFromDbTag() function in %s ", col.Default, typ.String(), "vdb/migrate/migrator.mssql.CreateTable.go")
				panic(err)
			}
